...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        console.error("Initialization Data Load Error:", error);
         // Don't overwrite specific error from default load if it happened
         if (!document.getElementById('config-status').classList.contains('error')) {
...
function initializeCytoscape() {
    // ... (ensure nodeLabelFunc is defined before calling this) ...
    // ... (rest of the function as previously corrected) ...
...
function initializeEditingExtensions() {
    ...
    // --- Register & Initialize Edge Handles (with delay) ---
    try {
        if (typeof cytoscapeEdgehandles === 'function' && !didRegisterEdgehandles) {
             ...
             // DEFER instance initialization slightly
             setTimeout(() => {
                 try {
                      if (!cy) { console.log("Edgehandles init deferred: cy became null."); return }; // Check cy again
                     ...
                     console.log("Edgehandles instance initialized (deferred).");
                     updateEditorPlaceholderText(); // Update placeholder after trying init
                 } catch(initError) {
                     console.error("Error initializing deferred Edgehandles instance:", initError);
                     edgeHandlesInstance = null;
    ...
    } catch (e) { console.error("Error during Edgehandles registration:", e); }
...
// ... (Keep ALL other functions defined previously: initializeUI, addNodeFromMenu, deleteElement, convertNodeType, markConfigUnsaved, clearUnsavedMark, loadDefaultConfig, addDefaultToDropdown, loadGraphData, saveConfiguration, loadConfigList, loadSelectedConfiguration, setDefaultConfiguration, deleteSelectedConfiguration, updateEditorPlaceholderText, etc.) ...
// --- UI Initialization ---
...
async function deleteSelectedConfiguration() {
    const s = document.getElementById('load-config-select');
    const id = s.value;
    const name = s.options[s.selectedIndex].text;
    if (!id || id === 'default-config-001') {
        setStatusMessage('config-status', "Select a saved config to delete.", "error");
        return;
    }
    // FIX: Corrected typo from &¤tConfig to && currentConfig
    if (!confirm(`Delete "${name}"?`)) return;
    setStatusMessage('config-status', `Deleting...`, "loading");
    showSpinner(true); enableUI(false);
    await retryFetch(async () => {
        const r = await fetch(`/api/configs/${id}`, { method: 'DELETE' });
        const d = await r.json();
        if (!r.ok) throw new Error(d.detail || `HTTP ${r.status}`);
        setStatusMessage('config-status', `Deleted '${name}'.`, "success");
        // Check if the deleted config *was* the currently loaded one
        if (currentConfig && currentConfig.id === id) { // Correct check
            if (defaultGraphStructure) {
                loadGraphData(defaultGraphStructure, true);
                setStatusMessage('config-status', `Deleted '${name}'. Default loaded.`, "success");
            } else {
                cy.elements().remove(); currentConfig = null; updateInputControls([]); clearSessionLog(); clearLLMOutputs(); document.getElementById('current-config-name').textContent = "None"; setStatusMessage('config-status', `Deleted '${name}'. Load another.`, "success");
            }
        }
        await loadConfigList(); // Refresh dropdown list
    }, 3, () => setStatusMessage('config-status', "Delete fail. Retry...", "error"))
    .catch(e => { console.error('Delete error:', e); setStatusMessage('config-status', `Delete failed: ${e.message}`, "error"); })
    .finally(() => { enableUI(true); showSpinner(false); });
}


// --- Other Utility Functions ---
function updateEditorPlaceholderText() { const p=document.querySelector('.graph-editor p'); if(!p)return; let m=""; const menusOk=contextMenuInstance!==null; const edgesOk=edgeHandlesInstance!==null; if(!menusOk&&!edgesOk) m="Editing unavailable: Failed libraries."; else if(!menusOk) m="Right-click menus disabled. Drag node handles to add edges."; else if(!edgesOk) m="Edge drawing disabled. Use right-click menus to edit."; else m="Right-click nodes/canvas to edit. Drag node handles for edges."; p.textContent=m; p.style.color=(menusOk&&edgesOk)?'#555':'orange'; }
