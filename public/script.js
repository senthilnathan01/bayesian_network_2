...
let sessionLog = [];
// let edgeHandlesInstance = null; // REMOVED
...
// ===========================================
// (Ensure ALL functions like nodeLabelFunc, simulateIsDag, initializeUI, fetchAndUpdateLLM, etc., are defined here first)
...
// *** NEW/REVISED Click-to-Connect Edge Functions ***
function startEdgeCreation(sourceNode) {
    if (!cy || !sourceNode || !sourceNode.isNode()) return;
    edgeSourceNode = sourceNode; // Store the source node globally
    setStatusMessage('config-status', `Edge started from ${sourceNode.id()}. Left-click target node...`, 'loading');
    sourceNode.addClass('edge-source-active'); // Add visual indication
    console.log("Edge creation started from:", sourceNode.id());
}
function handleNodeTap(event) {
    const targetNode = event.target;
    if (edgeSourceNode && edgeSourceNode.id() !== targetNode.id()) {
        const sourceId = edgeSourceNode.id(); const targetId = targetNode.id();
        if (cy.edges(`[source = "${sourceId}"][target = "${targetId}"]`).length > 0) {
            setStatusMessage('config-status', `Edge ${sourceId}->${targetId} already exists.`, "error");
        } else {
            const tempEdges = cy.edges().map(e => ({ source: e.source().id(), target: e.target().id() }));
            tempEdges.push({ source: sourceId, target: targetId });
            const tempGraph = { nodes: cy.nodes().map(n => n.data()), edges: tempEdges };
            if (!simulateIsDag(tempGraph)) {
                 alert(`Cannot add edge ${sourceId}->${targetId} (creates cycle).`);
                 setStatusMessage('config-status', `Edge aborted (creates cycle).`, "error");
            } else {
                 cy.add({ group: 'edges', data: { source: sourceId, target: targetId } });
                 console.log(`Added edge ${sourceId}->${targetId}`);
                 setStatusMessage('config-status', `Edge ${sourceId}->${targetId} added.`, "success");
                 // runLayout(); // Optional layout run
                 markConfigUnsaved();
            }
        }
        cancelEdgeCreation();
    } else if (edgeSourceNode && edgeSourceNode.id() === targetNode.id()) {
        cancelEdgeCreation(); // Clicked same node, cancel
    }
}
function handleCanvasTap(event) { if (edgeSourceNode) { cancelEdgeCreation(); } }
function cancelEdgeCreation() {
    if (edgeSourceNode) { edgeSourceNode.removeClass('edge-source-active'); }
    edgeSourceNode = null;
    setTimeout(() => {
        const currentStatusEl = document.getElementById('config-status');
        if(currentStatusEl && (currentStatusEl.textContent.includes('Edge started') || currentStatusEl.textContent.includes('aborted'))) {
             const msg = currentConfig ? `Config '${currentConfig.name}' loaded.` : "Ready.";
             setStatusMessage('config-status', msg , "success");
         }
    }, 1500);
}
// *** END of Click-to-Connect Edge Functions ***

// --- Configuration Management Functions ---
async function loadDefaultConfig() { console.log("Fetching default config..."); try { await retryFetch(async () => { const r = await fetch('/api/configs/default'); if (!r.ok) throw new Error(`HTTP ${r.status}`); defaultGraphStructure = await r.json(); console.log("Default stored."); }, 3); } catch (e) { console.error('Fetch default error:', e); defaultGraphStructure = null; throw e; } }
...
// --- Cytoscape Event Listeners (for click-to-connect) ---
function setupCytoscapeEventListeners() {
    ...
    console.log("Cytoscape tap listeners added for edge creation.");
}
