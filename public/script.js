...
function simulateIsDag(graph) {
    ...
    graph.edges.forEach(e => {
        if (nodesSet.has(e.source) && nodesSet.has(e.target)) {
             if(e.source in adj) adj[e.source].push(e.target);
             else adj[e.source] = [e.target]; // Initialize if source wasn't seen yet
        } else {
             console.warn(`SimulateIsDag skipped edge with missing node: ${e.source} -> ${e.target}`);
        }
    });
    ...
    function dfs(node) {
        ...
        for (const neighbor of adj[node] || []) {
            if (!nodesSet.has(neighbor)) continue; // Should not happen if edge check is done, but safe
            if (path.has(neighbor)) return false; // Cycle detected
            if (!visited.has(neighbor)) { if (!dfs(neighbor)) return false; } // Recurse
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    } catch (error) { console.error("Initialization Data Load Error:", error); setStatusMessage('config-status', `Initialization failed: ${error.message}`, "error"); if (!currentConfig) { updateInputControls([]); document.getElementById('current-config-name').textContent = "None"; } }
...
function initializeCytoscape() {
    ...
    } catch(error) { console.error("Failed to initialize Cytoscape Core:", error); alert(`Critical Error initializing graph library: ${error.message}`); cy = null; }
}
...
function initializeEditingExtensions() {
    ...
    try {
        if (typeof cytoscapeEdgehandles === 'function' && !didRegisterEdgehandles) {
             cytoscape.use( cytoscapeEdgehandles ); // REGISTER
            ...
            edgeHandlesInstance = cy.edgehandles({ // INITIALIZE instance
                 ...
                 complete: ( src, tgt, added ) => { console.log("Edge added:", added); const g={nodes:cy.nodes().map(n=>n.data()), edges:cy.edges().map(e=>({source:e.source().id(), target:e.target().id()}))}; if (!simulateIsDag(g)) { alert(`Cycle detected! Removing edge ${src.id()}->${tgt.id()}`); cy.remove(added); setStatusMessage('config-status',`Edge aborted (cycle).`,"error"); } else { markConfigUnsaved(); setStatusMessage('config-status',`Edge ${src.id()}->${tgt.id()} added.`,"success"); /* runLayout(); optional */ } }
            });
            console.log("Edgehandles instance initialized.");
        } else if (!didRegisterEdgehandles) { console.warn("cytoscape-edgehandles lib not found globally."); }
        else { console.log("Edgehandles already registered."); }
    } catch (e) { console.error("Error during Edgehandles registration/initialization:", e); }
    ...
    } catch (e) { console.error("Error during Context Menus registration/initialization:", e); }

     updateEditorPlaceholderText(); // Update message based on success
}
